...
test('ส่งของแบบรอตรวจ - ✅ รับของเพิ่มในตู้ - ❌ ย้อนยอด - ไม่บันทึกซ้ำ', () => {
  ...
  assert.equal(store.getGuild(guild).locker[0].quantity, 1000000);
  ...
  assert.equal(store.getGuild(guild).locker[0].quantity, 0);
});
test('การส่งใหม่แทนรายการถูกปฏิเสธ และบล็อกการตรวจรายการเก่า', () => {
  ...
  assert.equal(store.getGuild(guild).locker.find(x => x.name === 'เงิน').quantity, 500000);
});
...
test('การปรับยอดต่ำกว่ารายการรับแล้ว ไม่อนุญาตให้ย้อนรายการจนยอดติดลบ', () => {
  store.lockerEdit(guild, 'เงิน', 0);
  const entry = delivery.list(store.getGuild(guild), date)[0];
  assert.throws(() => delivery.review(guild,date,entry.id,'rejected','mod'), /ยอดตู้แก๊งไม่พอ/);
