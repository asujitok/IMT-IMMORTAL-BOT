...
module.exports = [
  ...
  new SlashCommandBuilder().setName('panel').setDescription('ผู้ดูแล: ส่งแผงปุ่มกดเช็กชื่อหรือเช็กของ')
    ...
    .addStringOption(o => o.setName('type').setDescription('ประเภทแผงปุ่ม').setRequired(true)
      .addChoices({ name: 'เช็กชื่อ', value: 'attendance' }, { name: 'เช็กของ', value: 'inventory' }, { name: 'ตู้แก๊ง', value: 'locker' })),
  ...
  new SlashCommandBuilder().setName('locker').setDescription('ตู้แก๊งส่วนกลาง [IMT] IMMORTAL')
    .addSubcommand(s => s.setName('panel').setDescription('สร้าง Dashboard และปุ่มตู้แก๊งในห้องปัจจุบัน'))
    .addSubcommand(s => s.setName('add').setDescription('เพิ่มของเข้าตู้แก๊ง (ถ้ามีชื่อเดิมจะบวกจำนวน)')
      .addStringOption(o => o.setName('name').setDescription('ชื่อของ').setRequired(true).setMaxLength(80))
    ...
    .addSubcommand(s => s.setName('edit').setDescription('แก้จำนวนและหน่วยของตู้แก๊ง')
      ...
      .addStringOption(o => o.setName('newname').setDescription('ชื่อใหม่ (ไม่บังคับ)').setMaxLength(80)))
    .addSubcommand(s => s.setName('remove').setDescription('ลบจำนวนออกจากตู้แก๊ง')
      .addStringOption(o => o.setName('name').setDescription('ชื่อของ').setRequired(true))
      .addIntegerOption(o => o.setName('quantity').setDescription('จำนวนที่ลบออก เว้นว่างคือลบรายการทั้งหมด').setMinValue(1)))
    .addSubcommand(s => s.setName('list').setDescription('ดู Dashboard ตู้แก๊ง'))
    .addSubcommandGroup(g => g.setName('role').setDescription('กำหนดยศที่สามารถแก้ไขตู้แก๊ง')
      .addSubcommand(s => s.setName('add').setDescription('เพิ่มยศผู้จัดการตู้แก๊ง')
        .addRoleOption(o => o.setName('role').setDescription('ยศที่อนุญาตให้เพิ่ม/ลบ/แก้ไขของ').setRequired(true)))
      .addSubcommand(s => s.setName('remove').setDescription('ลบยศผู้จัดการตู้แก๊ง')
        .addRoleOption(o => o.setName('role').setDescription('ยศที่ต้องการลบสิทธิ์').setRequired(true)))
      .addSubcommand(s => s.setName('list').setDescription('ดูยศผู้จัดการตู้แก๊ง'))),
